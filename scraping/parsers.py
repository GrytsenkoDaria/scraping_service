...
headers = [
    {'User-Agent': 'Mozilla/5.0 (Windows NT 5.1; rv:47.0) Gecko/20100101 Firefox/47.0',  # noqa
        'Accept':'text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8'},  # noqa
    {'User-Agent': 'Mozilla/5.0 (Windows NT 5.1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/49.0.2623.112 Safari/537.36',  # noqa
        'Accept':'text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8'},  # noqa
    {'User-Agent': 'Mozilla/5.0 (Windows NT 6.1; rv:53.0) Gecko/20100101 Firefox/53.0',  # noqa
        'Accept':'text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8'}  # noqa
    ]
...
def rabota(url):
    ...
    if resp.status_code == 200:
        soup = bs(resp.content, 'html.parser')
        new_jobs = soup.find(
            'div', attrs={'class': 'f-vacancylist-newnotfound'}
        )  # if there are new vacancies
        if not new_jobs:
            table = soup.find('table', id="ctl00_content_vacancyList_gridList")
            if table:
                tr_list = table.find_all('tr', attrs={'id': True})  # noqa finds all tr`s which have an id
                for tr in tr_list:
                    div = tr.find('div', attrs={'class': 'card-body'})
                    if div:  # if there is an ads there will be no 'card-body'
    ...
    if resp.status_code == 200:
        ...
        if main_div:
            ...
            for li in li_list:
                ...
                content = cont.text
                company = 'No name'
                comp_name = title.find('a', attrs={'class': 'company'})
                if comp_name:
...
if __name__ == '__main__':
    url = 'https://rabota.ua/zapros/python/%D0%BA%D0%B8%D0%B5%D0%B2'  # noqa
